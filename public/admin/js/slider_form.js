                ...
                success: function (data) {
                    if(data){
                        ...
                        for (var i = 0; i < length; i++) {
                            ...
                            $output += '<td class="text-center"> <a class="btn btn-primary btn-xs" id="'+data[i].id+'" onclick="editslider(this.id)" style="color: #fff"> <i class="fa fa-edit"></i> Edit </a> <a class="btn btn-danger btn-xs" name="'+data[i].id+'" onclick="deleteslider(this.name,event)" style="color: #ffffff"> <i class="fas fa-remove"></i> Delete </a></td>';
                            $output += '</tr>';
                        }
                        $("#frmCheckout").trigger("reset");
                        $("#show_photo").attr("src","http://localhost/School-event/public/admin/img/no_image_found.png");
                        $("#slider_id").val("");
                        $("#slider_list_data").empty();
                        $("#slider_list_data").append($output);
                    }
                },
                // cache: false,
                contentType: false,
...
{
    var APP_URL = $('meta[name="_base_url"]').attr('content');
    var message = confirm("Are you sure you want to Delete this Slider??");
    if(message == true) {
        $.ajaxSetup({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
