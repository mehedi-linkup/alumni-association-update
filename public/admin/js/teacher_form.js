...
function teachersubmit(){
    ...
    $("#frmCheckout").validate({
        ...
        messages: {
            ...
            degingnation: {
                required: "Please Enter Teacher Degingnation",
            },
        },
                ...
                success: function (data) {
                    if(data){
                        ...
                        for (var i = 0; i < length; i++) {
                            ...
                            $output += '<td class="text-center">' + data[i].qualification + '</td>';
                            $output += '<td class="text-center">' + data[i].depertment + '</td>';
                            $output += '<td class="text-center">' + data[i].degingnation_title + '</td>';
                            $output += '<td class="text-center">' + data[i].teacher_type + '</td>';
                            $output += '<td class="text-center">' +'<img height="60" width="110" src="'+locate+ (data[i].image) +'">'+ '</td>';
                            $output += '<td class="text-center"> <a class="btn btn-primary btn-xs" id="'+data[i].id+'" onclick="editteacher(this.id)" style="color: #fff"> <i class="fa fa-edit"></i> Edit </a> <a class="btn btn-danger btn-xs" name="'+data[i].id+'" onclick="deleteteacher(this.name,event)" style="color: #ffffff"> <i class="fas fa-remove"></i> Delete </a></td>';
                            $output += '</tr>';
                        }
                        $("#frmCheckout").trigger("reset");
                        $("#show_photo").attr("src","http://localhost/School-event/public/admin/img/no_image_found.png");
                        $("#teacher_id").val("");
                        $("#teacher_list_data").empty();
                        $("#teacher_list_data").append($output);
                    }
                },
                // cache: false,
                contentType: false,
...
function editteacher(id)
{
    ...
    var APP_URL = $('meta[name="_base_url"]').attr('content');
    $.ajax({
        url: APP_URL + '/edit-teacher',
        method: "get",
        dataType: "json",
        data: {id:id},
        success: function (data) {
            $("#name").val(data.name);
            $("#degingnation").val(data.qualification);
            $("#depertment").val(data.depertment);
            $("#teacher_type").val(data.degingnation_title);
            $("#teacher_title").val(data.teacher_type);
